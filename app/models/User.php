        ...
        if ($userId !== null) {
            ...
            $stmt = $this->db->prepare(
                'SELECT
                    ...
                    (SELECT COUNT(*) FROM likes l JOIN prompts p ON p.id = l.prompt_id WHERE p.user_id = :uid2) AS likes_received,
                    (SELECT COUNT(*) FROM saves s JOIN prompts p ON p.id = s.prompt_id WHERE p.user_id = :uid3) AS saves_received,
                    (SELECT COUNT(*) FROM views v JOIN prompts p ON p.id = v.prompt_id WHERE p.user_id = :uid4) AS views_received'
            );
            $stmt->execute(['uid1' => $userId, 'uid2' => $userId, 'uid3' => $userId, 'uid4' => $userId]);
            return $stmt->fetch() ?: ['prompt_count' => 0, 'likes_received' => 0, 'saves_received' => 0, 'views_received' => 0];
        }
